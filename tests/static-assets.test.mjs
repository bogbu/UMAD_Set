...
assert.equal(files.distIndex.includes('settings-27'), true, 'dist index should reference settings-27 assets');
...
for (const [name, content] of versionedFiles) {
  assert.match(content, /settings-27/, `${name} should reference the current settings-27 asset version`);
  assert.equal(content.includes('settings-26'), false, `${name} should not reference stale settings-26 assets`);
