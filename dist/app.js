...
const APP_BUILD_VERSION='resize-20';
const STORAGE_KEY='umad-p4-helper-state',PANEL_STORAGE_KEY='umad-p4-helper-panel-size',validPhases=['exdeath','chaos'];
const defaultPanelSize={width:620,height:520},minPanelSize={width:320,height:260};
...
function startPanelDrag(event){const shell=event.currentTarget.closest('.shell');if(!shell)return;event.preventDefault();const scale=viewportScale(shell);panelDrag={pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,startWidth:panelSize.width,startHeight:panelSize.height,scaleX:scale.scaleX,scaleY:scale.scaleY};event.currentTarget.setPointerCapture(event.pointerId);document.body.classList.add('resizing-panel')}
function movePanelDrag(event){if(!panelDrag||event.pointerId!==panelDrag.pointerId)return;setPanelSize({width:panelDrag.startWidth+(event.clientX-panelDrag.startX)/panelDrag.scaleX,height:panelDrag.startHeight+(event.clientY-panelDrag.startY)/panelDrag.scaleY})}
