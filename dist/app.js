...
function connectCombatEventSource(){try{createCombatEventSource().connect(ev=>{if(ev.type==='PhaseChanged'&&validPhases.includes(ev.phase)){state.phase=ev.phase;save()}})}catch(error){console.error('Failed to connect combat event source.',error)}}
