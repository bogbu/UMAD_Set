...
const APP_BUILD_VERSION='resize-15';
const STORAGE_KEY='umad-p4-helper-state',SIZE_STORAGE_KEY='umad-p4-helper-size',validPhases=['exdeath','chaos'];let state=load(),panelSize=loadPanelSize(),confirmReset=false,copyNotice='',resizeState=null;
...
function viewportLimit(value,fallback){return Number.isFinite(value)&&value>0?value:fallback}
function normalizePanelSize(size){const viewportWidth=viewportLimit(window.innerWidth,640),viewportHeight=viewportLimit(window.innerHeight,420);return{width:clamp(Number(size&&size.width)||420,320,Math.max(320,viewportWidth-20)),height:clamp(Number(size&&size.height)||360,220,Math.max(220,viewportHeight-20))}}
function readStoredPanelSize(){try{return JSON.parse(localStorage.getItem(SIZE_STORAGE_KEY)||'{}')}catch(error){console.error('Failed to read saved panel size.',error);return{}}}
function loadPanelSize(){return normalizePanelSize(readStoredPanelSize())}
function savePanelSize(){try{localStorage.setItem(SIZE_STORAGE_KEY,JSON.stringify(panelSize))}catch(error){console.error('Failed to save panel size.',error)}}
function panelStyle(){const size=normalizePanelSize(panelSize);return`style="--panel-width:${size.width}px;--panel-height:${size.height}px"`}
function resizePanelTo(width,height){panelSize=normalizePanelSize({width,height});savePanelSize();const shell=document.querySelector('.shell');if(shell){shell.style.setProperty('--panel-width',`${panelSize.width}px`);shell.style.setProperty('--panel-height',`${panelSize.height}px`)}}
...
function render(){const eye=calculateExdeathEyeActions(state.exdeath);const speedLabels={[MechanicState.Circle]:'빠름',[MechanicState.Question]:'느림'};app.innerHTML=`<main class="shell" ${panelStyle()}><header><div><h1>절요성 4페 컨페</h1></div><button class="reset" data-reset>${icon('reset')}${confirmReset?'한 번 더':'초기화'}</button></header><section class="mechanics">${row('water','물','exdeath.water',state.exdeath.water,calculateExdeathAction('water',state.exdeath.water),exdeathOrder('water'))}${row('lightning','번개','exdeath.thunder',state.exdeath.thunder,calculateExdeathAction('thunder',state.exdeath.thunder),exdeathOrder('thunder'))}${row('bomb','폭탄','exdeath.bomb',state.exdeath.bomb,calculateExdeathAction('bomb',state.exdeath.bomb),exdeathOrder('bomb'))}${row('eye','디버프','chaos.debuff',state.chaos.debuff,calculateChaosAction('debuff',state.chaos.debuff),0,speedLabels)}<div class="phase-gap" aria-hidden="true"></div>${row('fire','화염','chaos.fire',state.chaos.fire,calculateChaosAction('fire',state.chaos.fire))}${row('water','해일','chaos.tsunami',state.chaos.tsunami,calculateChaosAction('tsunami',state.chaos.tsunami))}${debuffSummary(eye)}</section><div class="resize-handle" data-resize-handle title="오른쪽 아래를 드래그해 크기 조절" aria-label="플러그인 크기 조절 핸들"></div></main>`;bind()}
function bind(){document.querySelectorAll('[data-set]').forEach(b=>b.onclick=()=>setPath(b.dataset.set,b.dataset.value));const resetButton=document.querySelector('[data-reset]');if(resetButton)resetButton.addEventListener('click',reset);const copyButton=document.querySelector('[data-copy-summary]');if(copyButton)copyButton.addEventListener('click',copySummary);const resizeHandle=document.querySelector('[data-resize-handle]');if(resizeHandle)resizeHandle.addEventListener('pointerdown',startResize)}
function startResize(event){event.preventDefault();const shell=document.querySelector('.shell');resizeState={pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,startWidth:shell?shell.offsetWidth:panelSize.width,startHeight:shell?shell.offsetHeight:panelSize.height};event.currentTarget.setPointerCapture(event.pointerId);document.body.classList.add('is-resizing')}
function dragResize(event){if(!resizeState||event.pointerId!==resizeState.pointerId)return;resizePanelTo(resizeState.startWidth+event.clientX-resizeState.startX,resizeState.startHeight+event.clientY-resizeState.startY)}
function stopResize(event){if(!resizeState||event.pointerId!==resizeState.pointerId)return;resizeState=null;document.body.classList.remove('is-resizing')}
window.addEventListener('pointermove',dragResize);window.addEventListener('pointerup',stopResize);window.addEventListener('pointercancel',stopResize);window.addEventListener('resize',()=>resizePanelTo(panelSize.width,panelSize.height));
