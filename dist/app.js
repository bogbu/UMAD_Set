...
class ActOverlayPluginEventSource{connect(onEvent){if(!window.addOverlayListener)return()=>{};const overlayLockPayload=data=>data&&data.detail?data.detail:data;const onChangeZone=()=>onEvent({type:'CombatStarted'});const onOverlayStateUpdate=data=>{const detail=overlayLockPayload(data);onEvent({type:'OverlayLockChanged',locked:!!(detail&&detail.isLocked)})};const onLogLine=data=>{const line=JSON.stringify(data).toLowerCase();if(line.includes('네오 엑스데스')||line.includes('neo exdeath'))onEvent({type:'BossDetected',boss:'neoExdeath'});if(line.includes('exdeath')||line.includes('엑스데스'))onEvent({type:'PhaseChanged',phase:'exdeath'});if(line.includes('chaos')||line.includes('케프카'))onEvent({type:'PhaseChanged',phase:'chaos'})};window.addOverlayListener('ChangeZone',onChangeZone);window.addOverlayListener('LogLine',onLogLine);document.addEventListener('onOverlayStateUpdate',onOverlayStateUpdate);if(window.callOverlayHandler){window.callOverlayHandler({call:'subscribe',events:['ChangeZone','LogLine']});const overlayState=window.callOverlayHandler({call:'getOverlayState'});if(overlayState&&overlayState.then)overlayState.then(onOverlayStateUpdate).catch(()=>{})}return()=>{if(window.removeOverlayListener)window.removeOverlayListener('ChangeZone',onChangeZone);if(window.removeOverlayListener)window.removeOverlayListener('LogLine',onLogLine);document.removeEventListener('onOverlayStateUpdate',onOverlayStateUpdate)}}}
