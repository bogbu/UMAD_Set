...
const APP_BUILD_VERSION='settings-24';
...
function debuffSummary(eye){const fire=calculateChaosAction('fire',state.chaos.fire,selectedPreset());const tsunami=calculateChaosAction('tsunami',state.chaos.tsunami,selectedPreset());const bomb=calculateExdeathAction('bomb',state.exdeath.bomb,selectedPreset());const debuff=calculateChaosAction('debuff',state.chaos.debuff,selectedPreset());const share=selectedShareAction();const chatLine=partyChatLine(eye);return`<button type="button" class="debuff-summary" data-copy-summary aria-label="첫 번째 줄 파티 채팅 복사: ${chatLine}" title="클릭하면 복사됩니다: ${chatLine}"><div class="summary-line summary-line-four"><b>${eyeText(eye[0])}</b><b>${fire}</b><b>${eyeText(eye[1])}</b><b>${tsunami}</b></div><div class="summary-line summary-line-two"><b>${bomb||'대기'}</b><b>${debuff}${share?` ${share}`:''}</b></div>${copyNotice?`<span class="copy-notice">${copyNotice}</span>`:''}</button>`}
...
function settingsPanel(){const presets=Object.values(ACTION_PRESETS);return`<div class="settings-popover" ${settingsOpen?'':'hidden'}><label>리스트 설정<select data-setting-preset>${presets.map(p=>`<option value="${p.id}" ${settings.presetId===p.id?'selected':''}>${p.name}</option>`).join('')}</select></label><label>오퍼시티 <span>${Math.round(settings.opacity*100)}%</span><input type="range" min="20" max="100" step="5" value="${Math.round(settings.opacity*100)}" data-setting-opacity></label></div>`}
