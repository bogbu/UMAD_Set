...
const STORAGE_KEY='umad-p4-helper-state',PANEL_STORAGE_KEY='umad-p4-helper-panel-size-v2',SETTINGS_STORAGE_KEY='umad-p4-helper-settings',validPhases=['exdeath','chaos'];
const overlayCanvasSize={width:700,height:800},defaultPanelSize={width:560,height:520},minPanelSize={width:0,height:260};
...
function availableOverlaySize(){const viewportWidth=Number(window.innerWidth)||overlayCanvasSize.width,viewportHeight=Number(window.innerHeight)||overlayCanvasSize.height;return{width:Math.max(0,Math.min(viewportWidth,overlayCanvasSize.width)),height:Math.max(0,Math.min(viewportHeight,overlayCanvasSize.height))}}
function clampPanelSize(size){const available=availableOverlaySize(),maxWidth=Math.max(minPanelSize.width,available.width-20),maxHeight=Math.max(minPanelSize.height,available.height-20);return{width:Math.min(Math.max(Math.round(Number(size&&size.width)||defaultPanelSize.width),minPanelSize.width),maxWidth),height:Math.min(Math.max(Math.round(Number(size&&size.height)||defaultPanelSize.height),minPanelSize.height),maxHeight)}}
...
function truth(path,value,labels,options,buttonClasses){const opts=options||[MechanicState.Circle,MechanicState.Question];return`<div class="truth-buttons">${opts.map(v=>`<button data-set="${path}" data-value="${v}" class="${[value===v?'selected':'',buttonClasses&&buttonClasses[v]?buttonClasses[v]:''].filter(Boolean).join(' ')}">${labels&&labels[v]?labels[v]:mark(v)}</button>`).join('')}</div>`}
...
function actionTone(action){const normalized=String(action||'').replace(/!/g,'');if(['산개','멈춰','나가'].includes(normalized))return'danger';if(['쉐어','움직여','그대로'].includes(normalized))return'safe';return''}
function actionTextClass(action,prefix='action'){const tone=actionTone(action);return tone?`${prefix}-${tone}`:''}
function actionTokenHtml(action,prefix='summary-action'){const className=actionTextClass(action,prefix);return className?`<span class="${className}">${action}</span>`:action}
function resultPillClass(result){return['result-pill',actionTextClass(result,'result')].filter(Boolean).join(' ')}
function row(iconName,label,path,value,result='',order=0,labels,options,buttonClasses){const hasResult=Boolean(result||order);return`<div class="mechanic-row ${hasResult?'':'no-result'}"><span class="mechanic-name">${icon(iconName)}${label}</span>${truth(path,value,labels,options,buttonClasses)}${hasResult?`<span class="result-wrap">${orderBadge(order)}${result?`<strong class="${resultPillClass(result)}">${result}</strong>`:''}</span>`:''}</div>`}
function gcDebuffButtonClasses(rowConfig){if(rowConfig.mechanic!=='debuff2')return null;const suggested=pairedGcDebuffChoices(state.custom.debuff1);return suggested.length?Object.fromEntries(suggested.map(choice=>[choice,'suggested'])):null}
function presetRow(rowConfig){const value=valueAtPath(rowConfig.path);const result=rowConfig.hideResult?'':rowConfig.pairPath?calculateGcDebuffAction(value,valueAtPath(rowConfig.pairPath),selectedPreset()):calculatePresetRowAction(rowConfig,value,selectedPreset());return row(rowConfig.icon,rowConfig.label,rowConfig.path,value,result,0,presetRowLabels(rowConfig),rowConfig.options,gcDebuffButtonClasses(rowConfig))}
...
function summaryActionHtml(action){return String(action||'대기').split(/(\s+)/).map(part=>part.trim()?actionTokenHtml(part):part).join('')}
function debuffSummary(eye){const actions=summaryFourActions(eye);const bottomActions=summaryBottomActions();const chatLine=partyChatLine(eye);return`<button type="button" class="debuff-summary" data-copy-summary aria-label="첫 번째 줄 파티 채팅 복사: ${chatLine}" title="클릭하면 복사됩니다: ${chatLine}"><div class="summary-line summary-line-four">${actions.map(action=>`<b>${summaryActionHtml(action)}</b>`).join('')}</div><div class="summary-line summary-line-two">${bottomActions.map(action=>`<b>${summaryActionHtml(action)}</b>`).join('')}</div>${copyNotice?`<span class="copy-notice">${copyNotice}</span>`:''}</button>`}
function setPath(path,value){const [group,key]=path.split('.');const presetRow=Array.isArray(selectedPreset().displayRows)&&selectedPreset().displayRows.find(row=>row.path===path);const normalized=presetRow&&presetRow.options&&presetRow.options.includes(value)?value:normalizeMechanicState(value);if(group==='exdeath'){setState(s=>({...s,exdeath:setExdeathMechanic(s.exdeath,key,value)}));return}setState(s=>{const nextGroup={...s[group],[key]:normalized};if(group==='custom'&&key==='debuff1'&&(normalized==='water'||normalized==='thunder'))nextGroup.debuff2='acceleration';return{...s,[group]:nextGroup}})}
